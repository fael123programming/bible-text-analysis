...
matthew = df[df['book'] == 'Matthew']
mark = df[df['book'] == 'Mark']
luke = df[df['book'] == 'Luke']
john = df[df['book'] == 'John']

dfs = [matthew, mark, luke, john]
...
for m in range(len(methods)):
    ...
    pd.DataFrame(df_data).to_csv(f'../datasets/gospel_fuzzy_comparative_analysis_{methods[m].__name__}.csv', index=False)
    print(f'Exported gospel_fuzzy_comparative_analysis_{methods[m].__name__}.csv')